...
from utils import imsaveTensor, VideoDataset
...
def apply_crf(frame_linear, crf_inv, device: torch.device):
    ...
    with cp.cuda.Device(device.index):
        ...
        y = cp.asarray(np.arange(0, 256, 1))
...
def main(argv=None):
    ...
    # Check if step 1 and 2 are done
    flag_step_1 = utils.check_log_step(config['DATA-GEN-PARAMS']['dp_log'], video_list[idx], 1)
    flag_step_2 = utils.check_log_step(config['DATA-GEN-PARAMS']['dp_log'], video_list[idx], 2)
    if not flag_step_1 or not flag_step_2:
        print('Step 1 or 2 is not done yet. Check dp log')
        return
    ...
    with torch.no_grad():
        for batchIdx, (frames, framesIds) in tqdm(enumerate(dataloader)):
            ...
            if (batchIdx % num_frames) == middle_frame_num:
                sharp = frames[0]
                filename = frames_name[framesIds[0].numpy()].split("_")[0] + ".png"
    ...
    # Update info log
    utils.update_log_step(config['DATA-GEN-PARAMS']['dp_log'], video_list[idx], 3)
