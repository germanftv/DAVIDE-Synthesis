...
4. Download the DAVIDE raw video captures:
    ```
...
5. Download the pretrained weights for XVFI. Refer to step 4 in the installation instructions provided in the [XVFI README](./davide_dp/XVFI/README.md).
...
- `CONFIG_FILE` (optional): The path to a custom configuration file. If not provided, the default configuration file `./davide_dp/configs/config.yaml` is used.
...
8. **Export annotated data**:
    ```
    conda activate DAVIDE-DP
    bash scripts/run_08_data_selection.sh 0
    ```
...
3. Save the images in PNG format within a folder and sort them. Additionally, create a text file listing the exposure times (one per line) corresponding to the sorted images.
